...
export const UserRow = React.memo(function UserRow({
  user, rangeStart, rangeEnd, timezone, isLast, scheduleEntries = [],
}: Props) {
  ...
  const { locEvents, ctxEvents, dotEvents, apiEvents } = useMemo(() => ({
    locEvents: user.events.filter((e) => e.category === "location" && e.event_type === "home"),
    ctxEvents: user.events.filter(
      (e) => e.category === "context" || (e.category === "activity" && e.ended_at),
    ),
    dotEvents: user.events.filter(
      (e) => e.category === "event" || (e.category === "activity" && !e.ended_at),
    ),
    apiEvents: user.events.filter((e) => e.category === "api_request"),
  }), [user.events]);
    ...
    <div className={`flex ${isLast ? "" : "border-b border-gray-100"}`} style={{ height: rowHeight }}>
      ...
      <div className="w-[220px] shrink-0 flex items-center px-3 border-r border-gray-200 bg-gray-50">
        ...
        <Link
          ...
          className="text-sm text-gray-700 truncate hover:text-indigo-600 transition-colors"
          title="스케줄 보기"
        >
          {user.user_name}
        </Link>
